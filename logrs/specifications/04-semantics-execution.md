...
解釈の結果は保持されず、値は文字列のままである (01-syntax-core の 1.1.5 を参照)
<<<
...
<<<
      ...
      note: <<<
correct_usage (例):
- 同一箇所に複数の出力項目が該当するとき、表示上 critical より high を優先する
...
<<<
  ...
  rule constraint label syntax {{
    placement: "constraint ラベルはブロック本体の末尾に置かなければならない"

    format: <<<
Markdown の箇条書き形式 ("-" で始まる行)
制約を箇条書きの項目として列挙する
<<<
