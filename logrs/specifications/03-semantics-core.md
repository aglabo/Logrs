...
**note**: 本節の `status` / `priority` / `acceptance` は、値集合の検証が意味論層で行われることを示すための **例示** です。コア仕様の状態語彙ではありません ([ドメイン状態の意味論](#ドメイン状態の意味論-プロファイルへ移動))。ドメインの値集合はプロファイルが定義します。
...
**note**: [04-semantics-execution の構文要素の意味](./04-semantics-execution.md#構文要素の意味) が `priority` ブロックの中で宣言する位階軸 `level` (critical >> high >> medium >> low) は、条件式の項に置けない軸です。位階として宣言するのは、衝突時にどちらが上位かを構造として残すためであって、実行条件の判定に使うためではありません。

**note**: 上の [値集合の意味論的検証](#値集合の意味論的検証) が宣言する位階軸 `priority` (A >> B >> C >> D >> E) と、04 の `level` は別の軸です。軸が違えば比較は未定義動作になります (`constraint rank undefined cases`)。同じ「優先度」という語を指していても、軸名が異なれば別の軸として扱ってください。
...
メタブロックは構文層での遮断によって自己再帰リスクを排除する:
  - ブロックの内側は地の文として読まれない (構文層での除外)
  - 除外の範囲は "{{" と "}}" の対応だけで決まる
<<<

  constraint exclusion layer distinction: <<<
- 構文層での除外はメタブロックだけが担う
- 解釈層での除外規則はコア仕様に存在しない
- ドメインが必要とする場合はプロファイルが定義する
<<<
...
効果範囲:
  - すべてのコマンド動作 (完全一致)
  - :session_phase 遷移、:execute_mode 遷移、副作用すべてを含む
...
- 意味論的差異・副作用の違いは存在しない
- :session_phase 遷移、:execute_mode 遷移、再入禁止挙動すべて一致
<<<
...
note は以下の特性を持つ:
  - 構文要素として認識される
  - 実行・制約・評価には一切影響しない
  - :session_phase 遷移・コマンド可否・条件分岐の判断材料に使用してはならない
